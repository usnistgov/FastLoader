...
class UnifiedViewData : public AbstractViewData<DataType>, public hh::MemoryData<UnifiedViewData<DataType>>{
private:
...
checkCudaErrors(cudaMallocManaged((void **) &this->data_, sizeof(DataType) * viewSize_));
  ...
  /// @brief Sychronizes the cudaEvent. Useful for ensuring data has been fully prefetched.
  ...
  void returnToMemoryManager() final {
    hh::MemoryData<UnifiedViewData<DataType>>::returnToMemoryManager();
  }
  ...
  /// @brief Recycles the unified memory, and prefeteches it back to the CPU before returning it to FastLoader's memory manager
