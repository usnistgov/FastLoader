...
class CachedTile {
 ...
 public:
  ...
  [[nodiscard]] size_t indexLayer() const { return indexLayer_; }
  ...
  [[nodiscard]] virtual size_t numberChannels() const { return numberChannels_; }
  ...
  void indexRow(size_t indexRow) { indexRow_ = indexRow; }
  /// @brief Tile column index setter
  /// @param indexCol Tile column to set
  void indexCol(size_t indexCol) { indexCol_ = indexCol; }
  /// @brief Tile depth index setter
  /// @param indexLayer Tile depth to set
  void indexLayer(size_t indexLayer) { indexLayer_ = indexLayer; }
  ...
  friend std::ostream &operator<<(std::ostream &os, const CachedTile &tile) {
    ...
    for(size_t layer = 0; layer < tile.tileDepth_; ++layer) {
      os << "Layer: " << layer << "\n";
      for (size_t row = 0; row < tile.tileHeight_; ++row) {
        os << "\t";
        for (size_t column = 0; column < tile.tileWidth_; ++column) {
          for (size_t channel = 0; channel < tile.numberChannels(); ++channel) {
            os << tile.data_->at(
